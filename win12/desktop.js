...
console.log('%cWindows 12', 'background-image: linear-gradient(to right,rgb(174, 115, 229),rgb(21, 105, 223)); border-radius: 8px; font-size: 1.3em; padding: 10px 15px; color: #fff; ');
// 好高级，还能这样？？
...
function disableIframes() {
    $('iframe:not(.nochanges):not([src*="Tetris"])').css('pointer-events', 'none');
    $('iframe:not(.nochanges):not([src*="Tetris"])').css('touch-action', 'none');
}

function enableIframes() {
    $('iframe:not(.nochanges):not([src*="Tetris"])').css('pointer-events', 'auto');
    $('iframe:not(.nochanges):not([src*="Tetris"])').css('touch-action', 'auto');
}
...
const cms = {
    ...
    'desktop': [
        ...
        `<a onmousedown="window.open(\'https://ae85.cn\',\'_blank\');" win12_title="https://ae85.cn" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><i class="bi bi-badge-3d-fill"></i> ${lang('查看其他项目','desktop.vogithub')}</a>`,
        arg => {
            if (edit_mode) {
        ...
        ['<i class="bi bi-info-circle"></i> '+lang('关于 Win12','about.name'), '$(\'#win-about>.about\').addClass(\'show\');$(\'#win-about>.update\').removeClass(\'show\');openapp(\'about\');if($(\'.window.about\').hasClass(\'min\'))minwin(\'about\');'],
...
const nts = {
    'about': {
        cnt: lang(`<p class="tit">Windows 12</p>
            <p>Windows 12 是一个开放源项目,<br />
...
2.关于win12:id为about;简介页面有关于本项目的介绍说明与贡献者信息，更新记录页面有本项目的各版本更新记录。
    ...
    send: (t, showusr = true, role='user') => {
        ...
        $.ajax({
                ...
                if (r.length) {
                    for (const i of r) {
                        if (/{openapp .+?}/.test(i)) {
                            ...
                            shownotice('feedback');
                            rt = rt.replace(i, '<div class="action"><p class="tit">反馈</p><p class="detail">关于 Windows 12</p></div>');
            ...
            error: function(error) {
                console.log(error);
                $('#copilot>.chat').append('<div class="line system"><p class="text">发生错误，请查稍后再试</p></div>');
                $('#copilot>.chat').scrollTop($('#copilot>.chat')[0].scrollHeight);
...
const date = {
    'zh-CN':`${da.getFullYear()}年${(da.getMonth() + 1).toString().padStart(2, '0')}月${da.getDate().toString().padStart(2, '0')}日  星期${['日', '一', '二', '三', '四', '五', '六'][da.getDay()]}`,
    'zh-TW':`${da.getFullYear()}年${(da.getMonth() + 1).toString().padStart(2, '0')}月${da.getDate().toString().padStart(2, '0')}日  星期${['日', '一', '二', '三', '四', '五', '六'][da.getDay()]}`,
...
function openapp(name) {
    ...
    if (apps[tmp]) {
        // 确保窗口元素存在后再加载应用内容
        const appWindow = document.querySelector('.window.' + name);
        if (appWindow) {
            if (apps[tmp].load && !apps[tmp].loaded) {
                apps[tmp].loaded = true;
                apps[tmp].load();
                // apps[tmp].init(); // init is called in load for tetris, ps, music
                $('.window.' + name).removeClass('load');
                return;
            }
            // 如果应用已经加载过，直接调用 init (对于需要重复初始化的应用)
            if (apps[tmp].init) {
                apps[tmp].init();
                 $('.window.' + name).removeClass('load');
                 return;
            ...
            console.warn(`Window element for ${name} not found in openapp. Retrying...`);
            // 如果窗口元素不存在，稍作延迟后重试 openapp
            setTimeout(() => openapp(name), 50);
            return;
        }
    ...
    setTimeout(() => {
        $('.window.' + name).removeClass('load');
        // 如果是俄罗斯方块游戏，确保 iframe 获得焦点
        if (name === 'tetris') {
            const iframe = document.querySelector('#win-tetris iframe');
            if (iframe) {
                iframe.focus();
...
function setIcon() {
    ...
    if (Array.isArray(JSON.parse(localStorage.getItem('desktop')))) {
        // Clear existing icons and add static ones
        $('#desktop')[0].innerHTML = '';

        const staticIcons = [
            {
                name: 'explorer',
                icon: 'apps/icons/explorer/thispc.svg',
                text: lang('此电脑','explorer.thispc'),
                onclick: "openapp('explorer');"
            },
            {
                name: 'setting',
                icon: 'icon/setting.svg',
                text: lang('设置','setting.name'),
                onclick: "openapp('setting');"
            },
            {
                name: 'edge',
                icon: 'icon/edge.svg',
        ...
        staticIcons.forEach(iconInfo => {
            const iconHtml = `
                <div class="b" appname="${iconInfo.name}">
                    <img src="${iconInfo.icon}">
                    <p>${iconInfo.text}</p>
                </div>
            `;
            $('#desktop')[0].insertAdjacentHTML('beforeend', iconHtml);
            const newIcon = $('#desktop').children().last()[0];

            // Bind double click event using addEventListener
            newIcon.addEventListener('dblclick', () => {
                 // Check if the corresponding window element exists before opening the app
                const targetWindow = document.querySelector('.window.' + iconInfo.name);
                if (targetWindow) {
                    eval(iconInfo.onclick); // Use eval to execute the onclick string
                } else {
                    console.warn(`Window element for ${iconInfo.name} not found. Cannot open app.`);
                     // Optionally, retry opening after a delay
                    setTimeout(() => eval(iconInfo.onclick), 100);
                }
            });
             newIcon.addEventListener('touchstart', () => {
                 // Check if the corresponding window element exists before opening the app
                const targetWindow = document.querySelector('.window.' + iconInfo.name);
                if (targetWindow) {
                    eval(iconInfo.onclick); // Use eval to execute the onclick string
                } else {
                    console.warn(`Window element for ${iconInfo.name} not found on touchstart. Cannot open app.`);
                     // Optionally, retry opening after a delay
                    setTimeout(() => eval(iconInfo.onclick), 100);
                }
            });
             newIcon.addEventListener('contextmenu', (event) => {
                showcm(event,'desktop.icon',[iconInfo.name,-1]);
            });
        });
...
document.getElementsByTagName('body')[0].onload = () => {
    ...
    // 添加网易云音乐窗口的点击事件处理，确保 iframe 获得焦点
    $('.window.music').on('click', function() {
        const iframe = this.querySelector('iframe');
        if (iframe) {
            iframe.focus();
            iframe.contentWindow.focus();
    ...
    // 在页面加载完成后绑定窗口拖动事件
    if (typeof bindWindowDragging === 'function') {
        bindWindowDragging();
    }
...
window.addEventListener('message', function(event) {
    // Check if the message is for closing the QQ window
    if (event.data && event.data.action === 'close-qq-window') {
        const qqWindow = document.getElementById('win-qq');
        if (qqWindow) {
            // Hide the QQ window by removing 'show' and adding 'hide'
            qqWindow.classList.remove('show');
            qqWindow.classList.add('hide');
        }
    }
});
