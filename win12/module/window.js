...
function showwin(name) {
    ...
    setTimeout(() => {
        ...
        // 在窗口完全显示后再尝试绑定拖动事件
        const targetWindow = document.querySelector('.window.' + name);
        // console.log(`Attempting to bind dragging for window: ${name}`);
        if (targetWindow) {
            const titbar = targetWindow.querySelector('.titbar');
            if (titbar) {
                console.log(`Found window element and titbar for ${name}. Binding dragging.`);
                if (typeof bindWindowDragging === 'function') {
                    bindWindowDragging(targetWindow);
                }
                 else {
                    // console.error('bindWindowDragging function is not defined.');
                }
            } else {
                // console.warn(`Titbar not found for window: ${name}`);
            }
        } else {
            // console.warn(`Window element not found for: ${name}`);
        }
    }, 0);
...
// 拖拽窗口
// 将拖动事件绑定逻辑封装到函数中，可以接受一个窗口元素作为参数
function bindWindowDragging(targetWindow = null) {
    const wins = targetWindow ? [targetWindow] : document.querySelectorAll('.window');
    const titbars = targetWindow ? [targetWindow.querySelector('.titbar')] : document.querySelectorAll('.window>.titbar');
    
    // 如果没有找到标题栏或者窗口，则不执行绑定
    if (wins.length === 0 || titbars.length === 0 || wins.length !== titbars.length) {
        console.warn('bindWindowDragging: 未找到匹配的窗口或标题栏', targetWindow);
        return;
    }
    ...
    // 将 mouseup, touchend, mousemove 事件绑定到 page 元素 (这些只需要绑定一次)
    // 检查是否已经绑定过，避免重复绑定
    if (!page.dataset.draggingEventsBound) {
        ...
        page.dataset.draggingEventsBound = 'true'; // 标记已绑定
    }
}
