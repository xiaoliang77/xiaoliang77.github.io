...
let apps = {
    ...
    qq: {
        init: () => {
            // Assuming a window element with id #win-qq exists in desktop.html
            $('#win-qq').removeClass('hide');
            $('#win-qq').addClass('show'); // Or whatever class is used to show windows
            focwin('qq'); // Assuming focwin exists to manage window focus
    ...
    ps: {
        init: () => {
            showwin('ps');
            bindWindowDragging(document.getElementById('win-ps')); // Add this line
            return null;
        },
        // load: () => {
        //     // Use a loop to wait for the element to be available
        //     const checkElementInterval = setInterval(() => {
        //         const psWindow = $('#win-ps')[0];
        //         if (psWindow) {
        //             clearInterval(checkElementInterval);
        //             psWindow.insertAdjacentHTML('afterbegin', '<iframe src="/apps/PS/index.html" frameborder="0" style="width: 100%; height: 100%;" loading="lazy"></iframe>');
        //         }
        //     }, 10); // Check every 10ms
    ...
    music: {
        init: () => {
            showwin('music');
            bindWindowDragging(document.getElementById('win-music')); // Add this line
            return null;
        },
        load: () => {
            // 调用系统的 showwin 函数来显示窗口
            showwin('music');
            // 如果需要，可以在这里添加其他 Photoshop 应用启动时的初始化代码
        },
    },
    run: {
        init: () => {
...
box-shadow: 0 1px 2px var(--s3d);
            ...
            <a class="a item act" ondblclick="window.open('https://pan.ae85.cn/','_blank')" ontouchend="window.open('https://pan.ae85.cn/','_blank')"
            oncontextmenu="showcm(event,'explorer.folder','Y:');return stop(event);">
            <img src="apps/icons/explorer/disk.svg"><div><p class="name">云盘 (Y:)</p><div class="bar"><div class="content" style="width: 41.5%;"></div>
            </div><p class="info">11.7 TB 可用, 共 20 TB</p></div></a></div>`;
...
(c) Microṡoft Corporation。保留所有杈利。
    ...
    search: {
        rand: [{ name: '薛定谔的待办事项.txt', bi: 'text', ty: '文本文档' },
            { name: '截图_忘记为什么截.png', bi: 'image', ty: 'PNG 文件' },
            { name: '最终版_v8_真的不改了.docx', bi: 'richtext', ty: 'Microsoft Word 文档' },
            { name: '表情包_关键时刻用得上.jpg', bi: 'image', ty: 'JPG 文件' },
            { name: '应急预案培训材料.pptx', bi: 'slides', ty: 'Microsoft Powerpoint 演示文稿' },
            { name: '预算表_超支预警.xlsx', bi: 'ruled', ty: 'Microsoft Excel 工作表' },
            { name: 'anner广告_临时用.svg', bi: 'image', ty: 'SVG 文件' },
            { name: '会议记录.doc', bi: 'richtext', ty: 'Microsoft Word 文档' }],
        search: le => {
            if (le > 0) {
                ...
                $('#search-win>.ans>.list>list').html(
					...
					setTimeout(() => {
						...
						<img src="icon/about.svg"><p>关于Win12</p></a>`);
    ...
    tetris: {
        init: function () {
            // console.log('Tetris init called.'); // Add this line
            // 初始化俄罗斯方块应用
            //showwin('tetris'); // Remove this line, we will call it after getting elements
            //bindWindowDragging(document.getElementById('win-tetris')); // Remove this line

            const tetrisWindow = document.getElementById('win-tetris');
            if (tetrisWindow) {
                 // 确保窗口显示
                showwin('tetris');
                // 确保标题栏存在并绑定拖动
                const titbar = tetrisWindow.querySelector('.titbar');
                 if (titbar) {
                    console.log('Binding dragging in tetris init.');
                    bindWindowDragging(tetrisWindow);
                 }
            }
            ...
            if (iframe) {
                ...
                window.addEventListener('keydown', function(e) {
                    if (document.querySelector('.window.tetris:not(.min)')) {
        ...
        load: function () {
            // console.log('Tetris load called.'); // Add this line
            // 加载俄罗斯方块应用
            const iframe = document.querySelector('#win-tetris iframe');
            if (iframe) {
