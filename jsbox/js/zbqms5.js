...
const mrhb = {
    ...
    events: {
        tapped: function (sender) {
            $app.openURL($cache.get("info").zfb)
        }
    },
    layout: function (make, view) {
    ...
    $ui.render({
        ...
        views: [
            ...
            },
            {
                ...
                props: {
                    id: "webyz",
                    html: `<head><meta charset="UTF-8"></head><body><span style="font-size:34px;"><h1>使用说明：</h1><h2>为了防止脚本恶意传播，密码将会定期修改，失效后重新到公众号获取即可。</h2><h1>密码获取：</h1><h2>前往公众号【VIP视频解析】在聊天窗口中回复〖903〗即可获取。<br></h2><br><h1><a href="http://t.cn/E7LC0v4">点击观看</a> 视频教程</h1> <h1><a href="https://iphone8.vip/fuli/xiaoliang.html">点击领取</a> 支付宝红包</h1><h1><a href="https://iphone8.vip/lxfs.html">关注公众号</a> VIP视频解析</h1><h2>by：iPhone 8、小良&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="https://iphone8.vip/">https://iphone8.vip/</a> </h2></span></body>`
                },
                layout: function (make) {
...
function getpt() {
    ...
    $ui.loading(true)
    $http.get({
        url: $text.base64Decode(key.replace(/lz/, "WF")) + "yuncaidan.txt",
        header: {
            "User-Agent": "5.0.1 rv:0.0.1 (iPhone; iOS 12.0; zh_CN)"
        },
        handler: function (resp) {
            $ui.loading(false)
            var json = resp.data.match(/\{(\s*.*?\s*)*\}/)[0]
            json = json.split("@msgType0")
            //    console.log(json)
            var data = []
            for (i in json) {
                var arr = json[i]
                if (arr.indexOf("address") !== -1) {
                    data.push({
                        tx: { src: arr.match(/@image(\S*?)\|/)[1] },
                        mc: { text: arr.match(/@name(\S*?)\|/)[1] + "(" + arr.match(/@number(\S*?)\|/)[1] + ")" },
                        id: arr.match(/@address(\S*?)\|/)[1]
                    })
                }
            }
            $("matrix").data = data
...
function getlist(id, mc) {
    ...
    $ui.loading(true)
    $http.get({
        url: $text.base64Decode(key.replace(/lz/, "WF")) + id,
        header: {
            "User-Agent": "5.0.1 rv:0.0.1 (iPhone; iOS 12.0; zh_CN)"
        },
        handler: function (resp) {
            $ui.loading(false)
            var json = resp.data
            json = json.split("@groupId")
            console.log(json)
            var data = []
            for (i in json) {
                var arr = json[i]
                if (arr.indexOf("@dzhttp") !== -1) {
                    data.push({
                        tx: { src: arr.match(/@tp(\S*?)\|/)[1] },
                        mc: { text: arr.match(/@mc(\S*?)\|/)[1] },
                        id: arr.match(/@dz(\S*?)\|/)[1]
                    })
                }
            }
