...
function runLongAction(action) {
  ...
  if (_hasPrefix(pattern, "editPhoto:")) {
    var decoder = require("./js-action/qr-decoder");
    decoder.csh();
    return;
  }
  ...
  if (_hasPrefix(pattern, "pin:")) {
    var download = require("./js-action/download");
    download.run();
    return;
  }
  ...
  if (_hasPrefix(pattern, "url_convert")) {
    var gtrans = require("./js-action/text-zhan");
    let text = $clipboard.text ? $clipboard.text : "";
    gtrans.gtrans(text)
    return;
  }
...
function runAction(action) {
  ...
  if (_hasPrefix(pattern, "pushbullet:")) {
    var pushbullet = require("./js-action/airtable");
    pushbullet.run();
    return;
  }
  ...
  if (_hasPrefix(pattern, "renmb:")) {
  var keyboard = require("./js-action/renmb");
    keyboard.run();
    return;
  }
  ...
  if (_hasPrefix(pattern, "compose://")) {
    ...
    var extension = $objc("NSExtension").invoke("extensionWithIdentifier:error:",
      identifier,
      null
    );
...
function tcn(url) {
  $ui.toast("生成中..", 10);
  $http.get({
    url: "http://api.weibo.com/2/short_url/shorten.json?source=2849184197&url_long=" +$text.URLEncode(url),
...
function unu(url) {
  $ui.toast("生成中..", 10);
  $http.get({
    url: "https://u.nu/api.php?action=shorturl&format=simple&url=" + $text.URLEncode(url),
    handler: function(resp) {
      var data = resp.data;
