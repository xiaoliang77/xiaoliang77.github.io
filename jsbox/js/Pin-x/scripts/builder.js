...
function createClipboardView() {
  return {
    ...
    events: {
      ...
      //   saveTextItems(mode);
      // },
      didLongPress: function (sender, indexPath, data) { //长按
        // console.log(mode);
        ...
        if (mode == "clip") {
          if ($app.env == $env.today) {
            ...
            // dataManager.set_yunduan(data.label.text, 1, indexPath.row)
          }
        } else {
          // 删除云端内容
          dataManager.set_yunduan(data.label.text, 2, indexPath.row)
        }
