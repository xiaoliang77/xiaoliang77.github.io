...
const pz = {
  title: "小良 - 更新器 2.2",
  pin: "pin://install?url=",
  jsbox: "jsbox://install?url=",
  jsboxx: "jsbox://import?url=",
  anzsb: "安装失败！\n请检查你的网络是否正常",
...
function refetch() {
  $ui.loading(true)
  $http.get({
    url: "https://gitee.com/yao07/update_device/raw/master/pin.json",
...
function azjs(jsurl) {
  ...
  if (appid == "app.cyan.pin") {
    ...
    $ui.toast("正在安装中 ...")
    if (appbb >= "1.28.0") {
      $app.openURL(pz.jsboxx + jsurl)
    } else {
      $app.openURL(pz.jsbox + jsurl)
    }
...
function render() {
  var json = $cache.get("stories").data
  $("vlist").data = [clzyli(json.jb, "脚本", "1"), clzyli(json.gz, "规则", "2"), clzyli(json.jc, "教程", "3"), clzyli(json.yy, "应用", "4"), clzyli(json.jx, "解析", "5"), clzyli(json.qt, "关于作者", "6")]
}
              ...
              didSelect: function(sender, indexPath, data) {
                if (data.url.indexOf(".js&name=") !== -1) {
                  ...
                  if (data.url.indexOf(":\/\/") == -1) {
                    var url = "workflow://import-workflow/?url=https%3A%2F%2Fworkflow.is%2Fworkflows%2F" + data.url + ".wflow&name=" + encodeURI(data.title)
                    $app.openURL(url)
                  } else {
